# HR-Attrition-Dashboard-(PowerBI)
# 👥📉 HR Atttrition Dashboard
<a href="#">Click Here To View Project</a>

## 📌Overview:
I have created an interactive HR Attrition Dashboard to analyze the employee attrition trends and identify key factors influencing employee turnover. The dashboard provides insights of attrition by department, age-group, gender, salary range and tenure group.

## 🛠 Tools Used
- Power BI (Visualization, Power query, DAX)
- Ms-Excel (Data source)

## 🎯 Goal and 🚧 Challenges:
The goal was to design a dashboard that tracks employee turnover trends and help the HR teams make data-driven decisions for improving employee retention using an interactive dashboard. The challenge faced was handling and organizing the data for clear, interactive visuals.

## 🔧 Key Features:
- KPIs for Total Employees, Attrition Rate, Total male, Total Female, Average Salary, Average Years at Company.
- Department wise attrition analysis.
- Age wise attrition
- Gender wise attrition
- Attrition by Job Role
- Attrition by Salary Range
- Attrition by Tenure Group

## 💡Impact:
-Provided actionable insights to reduce employee attrition
- Improved HR reporting efficiency through interactive visualization
- Supported strategic HR planning using data-driven insights

## 🔗 Contact
- 📧 [Mail: Ajaytamang2130@gmail.com]  
- 🌐 [LinkedIn: https://www.linkedin.com/in/ajaytamang1/]
